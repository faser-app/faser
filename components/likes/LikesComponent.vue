<template>
    <div
        class="flex mb-4 justify-center text-center"
    ...
    <div
        v-show="category === 'post'"
        class="rounded-md p-2 mx-2 mb-2"
    ...
    >
        ...
        <div
            v-else
            v-for="(community, index) in communityLoaded"
            :key="community"
            class="w-full flex justify-center"
        >
            <PostGetPostComponent
...
document.addEventListener('scroll', (event) => {
    if (document.body.offsetHeight - 8000 < window.scrollY) {
        if (
            lastRequest.value + 1000 < Date.now() &&
            category.value === 'post'
        ) {
        ...
        if (
            communityLastRequest.value + 1000 < Date.now() &&
...
function loadPosts(postsToLoad) {
    for (let i = 0; i < postsToLoad; i++) {
        if (
            postIndex.value < postsValue.value.length &&
            category.value === 'post'
        ) {
        ...
        if (
            communityIndex.value < communityValue.value.length &&
            category.value === 'community'
        ) {
            communityLoaded.value.push(
                communityValue.value[communityIndex.value]
            )
