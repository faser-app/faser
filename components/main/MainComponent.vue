<template>
    ...
    <div
        class="min-h-[calc(100vh-4.5rem)] md:flex px-2 text-white"
        :style="{ backgroundColor: currentPalette.bg }"
    >
        <div
            class="w-full rounded-md md:px-4"
        ...
        >
            <div v-if="loggedIn" class="mb-4">
                ...
                <PostCompactCreatePostComponent
                    :ownProfile="ownAccountData"
            ...
            <div v-else-if="loggedIn && noPosts && loaded">
                <div
                    class="w-full min-h-[calc(100vh-4.5rem)] flex items-center justify-center"
                >
                    <div class="text-center">
                        <h1
                            class="text-3xl font-bold text-transparent bg-linear-to-tr from-[#24c7ce] to-[#1ed794] bg-clip-text"
                        >
                            No posts available
                        </h1>
                        <p class="mb-4">
                            You have to follow some people to see posts on your
                            home page
                        </p>
                    </div>
                </div>
            ...
            <div v-else>
                ...
                >
                    <div class="text-center">
                        ...
                        <p class="mb-4">
                            You have to be logged in to view and make posts.
                        </p>
                        <RouterLink
                            to="/login"
                            class="py-3 px-5 rounded-md bg-gray-800"
                            >Login</RouterLink
                        >
                    </div>
                </div>
    ...
    <div
        class="blob bg-linear-to-tr z-1 pointer-events-none from-[#24c7ce] to-[#1ed794]"
    ></div>
</template>
...
function loadPosts() {
    if (Cookies.get('token')) {
        if (!noMorePosts.value) {
            loading.value = true
            axios
                .post(baseURL + '/api/posts/loadPosts', {
