<template>
    <div class="min-h-[calc(100vh-4.5rem)] px-4 py-6"
        ...
        <div v-if="participants.length === 0" class="flex flex-col items-center justify-center py-10">
            ...
            <p class="text-gray-500 text-sm text-center mt-1">Messages sent to you will appear here</p>
        </div>

        <!-- Conversation list -->
        <div class="space-y-2">
            <div v-for="participant in participants" :key="participant[0].id"
                class="rounded-xl transition-all hover:bg-opacity-80"
                :style="{ backgroundColor: currentPalette.bgSecondary }">
                <RouterLink :to="'/messages/' + participant[0].id" class="flex items-center p-4 w-full">
                    <!-- Profile picture -->
                    <div class="relative">
                        <div
                            ...
                            <img v-if="haveProfilePicture"
                                ...
                                class="w-12 h-12 object-cover" @error="haveProfilePicture = false" />
                            <i v-else class="fa-solid fa-user text-2xl"></i>
                        </div>
                        <div v-if="isOnline(participant[0].id)"
                    ...
                    <div class="ml-3 flex-1 overflow-hidden">
                        <div class="flex items-center">
                            <p class="font-medium truncate">{{ participant[0].displayName }}</p>
                            <!-- Verification badge if user is verified -->
                            <div v-if="participant[0].businessAccount"
                                class="ml-2 flex justify-center items-center bg-yellow-600 border border-yellow-300 rounded-full w-5 h-5">
                                <i class="fa-solid fa-check text-xs"></i>
                            </div>
                            <div v-else-if="participant[0].verifiedAccount"
                                class="ml-2 flex justify-center items-center bg-sky-600 border border-sky-300 rounded-full w-5 h-5">
                                <i class="fa-solid fa-check text-xs"></i>
                            </div>
                        </div>
                        <div class="flex justify-between">
                            <p class="text-gray-400 text-sm truncate">@{{ participant[0].username }}</p>
                            <p class="text-gray-500 text-xs" v-if="getLastMessageTime(participant[0].id)">
...
onMounted(async () => {
    try {
        const response = await axios.post(baseURL + "/api/social/getDirectMessages", {
        ...
        const messagePromises = participantsIds.map(userId =>
            axios.post(baseURL + "/api/messages/getDMs", {
                token: Cookies.get("token"),
                otherAccount: userId,
                ...
                .then(messageRes => {
                    if (messageRes.data.messages && messageRes.data.messages.length > 0) {
                        lastMessages.value[userId] = messageRes.data.messages[0].time;
                    }
                })
                .catch(() => { })
        );
        ...
        participants.value.sort((a, b) => {
            const timeA = lastMessages.value[a[0]?.id] || '1970-01-01T00:00:00.000Z';
            const timeB = lastMessages.value[b[0]?.id] || '1970-01-01T00:00:00.000Z';
