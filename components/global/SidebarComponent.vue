<template>
    <div class="sidebar-container h-screen fixed left-0 top-0 flex flex-col"
        :style="{ backgroundColor: currentPalette.bgSecondary }">
        <!-- Logo -->
        <RouterLink to="/" class="px-3 py-4 flex items-center justify-center md:justify-start">
            <div class="h-10 w-10 flex items-center justify-center rounded-full">
                <div v-if="currentPalette.name !== 'normal'" class="rounded-full" :style="{
          ...
          height: '40px'
            ...
            <span class="hidden md:block text-xl font-bold ml-4 text-white">faser</span>
        </RouterLink>
        ...
        <div class="mt-4 flex flex-col flex-1">
            <nav>
                <div v-for="navItem in navigationItems" :key="navItem.name">
                    <RouterLink :to="navItem.href" class="nav-link" :class="{'active': isActive(navItem.href)}">
                        <div
                            ...
                            <span class="hidden md:block ml-4">{{ navItem.name }}</span>
                        </div>
                    </RouterLink>
                </div>
            </nav>
                    ...
                    <MenuButton class="w-full">
                        <div
                            ...
                            <div class="hidden md:block ml-3 text-left">
                                <p class="text-white font-medium text-sm truncate max-w-[100px]">{{ displayName }}</p>
                                <p class="text-gray-400 text-xs truncate max-w-[100px]">@{{ username }}</p>
                            </div>
                            <i class="hidden md:block fa-solid fa-ellipsis-h text-gray-400 ml-auto"></i>
                        </div>
                    </MenuButton>
    ...
    <transition name="fade">
        <div v-if="showPostModal" @click.self="showPostModal = false"
            ...
            <div class="w-full max-w-lg p-4 rounded-xl shadow-lg" :style="{ backgroundColor: currentPalette.bg }">
                ...
                <PostCreatePostComponent @post-created="closePostModal" text="Post" :mobile="false"
