<template>
    <Menu as="div" class="relative w-full inline-block text-left z-2">
        <div>
            <MenuButton
                class="group flex w-full text-red-500 items-center rounded-md text-sm"
            >
        ...
        >
            <MenuItems class="">
                <div class="px-1 py-1">
                    ...
                    >
                        <button
                            @click="switchAccount(tokenList[index])"
                            :class="[
                                active
...
function logout() {
    ...
    if (tokenList) {
        try {
            ...
            Cookies.set('tokenList', JSON.stringify(filterTokenList()), {
                expires: 365,
            })
        } catch (error) {
