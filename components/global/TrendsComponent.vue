<template>
    <div
        class="trends-container h-screen overflow-y-auto px-4 py-4 text-white"
    ...
    >
        ...
        <div class="search-wrapper mb-6">
            <div class="search-container relative">
                <div class="search-input-wrapper relative">
                    <i
                        class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-500"
                    ></i>
                    <input
                        type="text"
                ...
                >
                    ...
                    >
                        <div class="flex items-center">
                            <div
                                class="profile-image-container h-10 w-10 rounded-full overflow-hidden mr-3"
                            >
                                <img
                            ...
                            <div class="user-info flex-grow">
                                <div class="flex items-center">
                                    ...
                                    <div
                                        v-if="user.businessAccount"
                                        class="ml-1"
                                    >
                                        <div
                                            class="verified-badge bg-yellow-600 border-yellow-300"
                                        >
                                            <i
                                    ...
                                    <div
                                        v-else-if="user.verifiedAccount"
                                        class="ml-1"
                                    >
                                        <div
                                            class="verified-badge bg-sky-600 border-sky-300"
                                        >
                                            <i
                                                class="fa-solid fa-check text-[10px]"
                                            ></i>
                                    ...
                                    <div
                                        v-if="user.privateAccount"
                                        class="ml-1"
                                    >
                                        <i
                                            class="fa-solid fa-lock text-gray-400 text-sm"
                                        ></i>
                                    </div>
                                </div>
        ...
        <div
            class="follow-section rounded-lg overflow-hidden"
            :style="{ backgroundColor: currentPalette.bg }"
        >
            <h2 class="text-xl font-bold p-4">Who to follow</h2>
            <div
                v-for="(suggestion, index) in followSuggestions"
                :key="index"
                class="suggestion-item p-4 hover:bg-gray-800 cursor-pointer flex items-center justify-between"
            >
                <div
                    class="flex w-full"
                    @click="navigateToUser(suggestion.username)"
                >
                    <div class="flex-shrink-0">
                        <div
                            class="h-12 w-12 rounded-full overflow-hidden"
                            :style="{
                                backgroundColor: suggestion.hasProfilePicture
                                    ? 'transparent'
                                    : '#1118276c',
                                border: suggestion.hasProfilePicture
                                    ? 'none'
                                    : '1px solid rgba(150, 150, 150, 0.15)',
                            }"
                        >
                            <img
                                v-if="suggestion.hasProfilePicture"
                                :src="
                                    'https://s3.faser.app/profilepictures/' +
                                    suggestion.id +
                                    '/image.png'
                                "
                                class="h-full w-full object-cover"
                                @error="suggestion.hasProfilePicture = false"
                            />
                            <div
                                v-else
                    ...
                    <div class="mx-3 flex-grow">
                        <div class="flex items-center">
                            ...
                            <div
                                v-if="suggestion.verifiedAccount"
                                class="verified-badge ml-1"
                            >
                ...
                <button
                    v-if="!suggestion.isFollowing"
                    class="follow-button px-4 py-1.5 rounded-full font-bold"
                ...
                <button
                    v-else
                    class="following-button px-4 py-1.5 rounded-full font-bold"
                    :style="{
    ...
    searchTimeoutId.value = setTimeout(() => {
        ...
        axios
            ...
            .then((response) => {
                ...
                searchResults.value = sortByLetterPriority(
                    userData,
...
function followUser(username, index) {
    axios
        .post(baseURL + '/api/social/followUser', {
...
onMounted(() => {
    ...
    axios
        .post(baseURL + '/api/profile/whoToFollow', {
            token: Cookies.get('token'),
        })
        .then((response) => {
