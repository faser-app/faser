<template>
    ...
    <div class="absolute -mt-8 mx-2 bg-gray-800 p-2 w-8 h-8 flex items-center justify-center rounded-full text-red-500 cursor-pointer shadow-lg"
    ...
    <!-- Message bubble -->
    <div class="p-3 rounded-2xl max-w-[80%] shadow-sm" @mouseenter="hover = true" @mouseleave="hover = false" :style="message.sender === ownProfile[0].id ? 
            { backgroundColor: currentPalette.buttonPrimary } : 
            { backgroundColor: currentPalette.bgSecondary }">

        <!-- Message content -->
        <p class="break-words ph-no-capture text-sm" v-html="convertedToUrl(message.message)"></p>

        <!-- Message timestamp -->
        <div class="w-full flex mt-1" :class="{
