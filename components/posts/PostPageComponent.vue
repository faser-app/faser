        ...
        <div v-if="isLoading" class="text-center mt-5">
            <div class="w-full justify-center">
                ...
                <div class="mt-5 rounded-xö flex justify-center">
                    <div
                        class="flex flex-col max-w-[90rem] w-full overflow-hidden rounded-md"
                    >
                        <div v-for="i in 10">
                            <PostFakePostComponent />
                        </div>
                ...
                <div class="mt-5 rounded-xö flex justify-center">
                    ...
                    >
                        ...
                        >
                            <PostGetPostComponent
                                :postId="comment"
                                :ownProfile="ownProfileArray[index]"
                                :ownProfileData="ownProfileData"
                                :account="accountData"
                            />
                        </div>
...
onMounted(async () => {
    try {
        ...
        for (let i = 0; i < postValue.data[0].comments.length; i++) {
            const commentValue = await axios.get(
                baseURL + '/api/social/fetchPost',
                {
                    headers: {
                        token: Cookies.get('token'),
