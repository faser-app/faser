<template>
    <BaseModalComponent :isOpen="showModal" @close="$emit('close')" title="Download Data" icon="id-card"
    ...
    <BaseModalComponent :isOpen="showDownloadLink" @close="showDownloadLink = false" cancel-text="Close"
        submit-text="Delete" @submit="deleteFile " title="Your Download Link">
        <div class="flex justify-center text-center mt-2 text-gray-400">
            <div class="md:w-96 w-full">
                <p>Here is your download link:</p>
                <a :href="dataResponse.url" target="_blank" class="text-blue-500 hover:underline mt-2">
                    {{ dataResponse.url }}
                </a>

                <p class="mt-2">This file was requested on: {{ new Date(dataResponse.requestedAt).toLocaleDateString() }}</p>
                <p>If you want to get an up to date version, you have to delete the current one and request a new File.</p>
            </div>
        </div>
    </BaseModalComponent>
</template>
...
watch(
    () => props.showModal,
    (value) => {
        axios.post(baseURL + "/api/download/getCurrent", {
