<template>
    <div class="min-h-[calc(100vh-4.5rem)]">
        <div
            v-if="community"
            class="mb-2 min-h-[calc(100vh-4.5rem)] w-screen flex items-center justify-center"
        >
            <div
                class="bg-gray-800 md:flex p-5 rounded-md w-[80svw] max-w-[90rem]"
            >
                <div
                    class="md:w-1/2 text-center items-center h-full flex flex-col justify-center"
                >
                    ...
                    <div class="mt-3">
                        ...
                        <div
                            ...
                            :class="{
                                'mr-2': index !== community.tags.length - 1,
                            }"
                        >
                            {{ tag }}
                        </div>
                    ...
                    <div class="mt-3">
                        <p
                            class="text-gray-400"
                            v-if="community.nsfw || community.private"
                        >
                            General info
                        </p>
                ...
                <div
                    class="md:w-1/2 text-center flex flex-col items-end justify-center"
                >
                    <img
                        :src="
                        ...
                        v-if="haveImage && community?.name"
                        class="w-full rounded-full"
                    />
                    <img
                        src="https://faser.app/icon/logo.png"
                        alt="Faser Logo"
                        class="w-full"
                        v-else
                    />
...
onMounted(() => {
    axios
        .post(baseURL + '/api/community/getCommunityByInvite', {
        ...
        .then((response) => {
            ...
            localeCreationDate.value = DateTime.fromSeconds(
                community.value.created / 1000
            ).toLocaleString()
        })
})
...
function joinCommunity() {
    axios
        .post(baseURL + '/api/community/joinCommunityByInvite', {
            inviteCode: props.inviteCode,
